...
public class ElementSmithingScreen extends HandledScreen<ElementSmithingScreenHandler> {
    ...
    protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
        ...
        if (!this.handler.getResult().isEmpty()) {
            ...
            Text cost = Text.translatable("content.eleworld.modify_elements_screen.cost", this.handler.getRequiredLevel());
    ...
    private Text getTooltip() {
        ...
        if (stack.getElements().size() >= Element.getRegistrySet(false).size() && !stack.isEmpty()) {
            tooltip = Text.translatable("content.eleworld.modify_elements_screen.full");
        } else if (player.experienceLevel < 1 && !stack.isEmpty() && material.isOf(Items.DIAMOND) && !player.isCreative()) {
            tooltip = Text.translatable("content.eleworld.modify_elements_screen.not_enough_level");
            ...
            tooltip = Text.translatable("content.eleworld.modify_elements_screen.different_item");
        }
        return tooltip;
    }
