...
public class ElementRendererGui {
    ...
    public void onRenderGameOverlayPost(MatrixStack matrixStack) {
        ...
        List<Element> elements = getElements(living);
        drawElements(matrixStack, elements);
    }

    @NotNull
    private static List<Element> getElements(LivingEntity living) {
    ...
    private void drawElements(MatrixStack matrixStack, List<Element> elements) {
        for (int i = 0; i < elements.size(); i++) {
            if (i > 15) break;
            Element element = elements.get(i);
            drawElement(element, i, matrixStack, elements.size());
        }
    }
    ...
    private void drawElement(Element element, int index, MatrixStack matrixStack, int size) {
        Item item = element.getFragmentItem();
        int y = mc.getWindow().getScaledHeight() / 2 + 16;
